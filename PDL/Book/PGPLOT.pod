...
C<perldl> command line, however it is error-prone in scripts (imagine
...
plot on top of the plotting area. If you want to make a new plot, you
...
PGPLOT has two disjoint sets of colours. One set determines the colour
...
The plot commands do not always lend themselves to easy broadcasting
...
these if there are fewer options than for instance, lines to draw (a
common occurrence if you wanted to draw a B<lot> of lines).

That said the C<PDL::Graphics::PGPLOT> interface does have limited
support for broadcasting in the C<line> and C<points> functions. These call
...
=head3 Opening a plot object and plotting a simple plot
...
summary of that company. You are only limited by your imagination!
...
the standard method should suffice.
...
Pearson's WWW page: L<http://astro.caltech.edu/~tjp/pgplot/>. This is

...
C-language PDL functions that follow the PDL broadcasting rules, without
having to write tedious loops or glue code.  You can write simple
computations with zero or more active dimensions (see
L<PDL::Book::Broadcasting>), write functions that contain a mix of Perl
...
way the new method should interact with the broadcasting engine (e.g.
...
The C<Pars> key specifies the B<ndarray> arguments for your function. It
...
demonstrates one such notation: to access the value in an ndarray, you must
...
all the PDL::PP code strings delimit ndarrays using dollar-signs (like C<$a()>
...
C<Pars> key. The C<Code> block makes use of these two ndarrays, printing out
...
debug, but these functions would be far more useful if they returned ndarrays
...
ndarrays in-place. For example, this function increments an ndarray:
...
If you want to modify an ndarray in-place, PDL provides multiple
...
a given ndarray. But I'm getting a bit ahead of myself. Generally speaking,
you shouldn't modify an ndarray in-place: you should return a result instead.
...
the output ndarrays as arguments, but I'll explore that in one of the
...
Mean when you give it all manner of ndarrays. Rather than bore you to death
...
The PP code that I present puts all the output ndarrays at the end of the
...
function. For example, instead of calling C<my_sum_and_diff> like this:
 ...
 # No output ndarrays in function call
 my ($sum, $diff) = $left->my_sum_and_diff($right);
...
zero-dimensional ndarrays. In this section, I will explain how to write
...
values in an ndarray for you. What if you want to do some sort of aggregate
behavior, such as computing the sum of all the values in an ndarray? This
...
that you want it to present the data from the input and output ndarrays as
...
I<broadcasting>. There are many calculations you cannot do with this simplistic
...
Notice that I have put something within the parentheses of the input ndarray,
...
one-dimensional ndarray, it returns a single result with the sum of all the
...
L<https://en.wikipedia.org/wiki/Matrix_multiplication> .) How
would we write such an algorithm using PDL::PP? First, the C<Pars> section
needs to indicate what sort of input and output
ndarrays we want to handle. The length of the row of the first matrix has to
...
relationship of the dimension sizes, the broadcasting engine just Does What You
...
=head3 Broadcastloops

PDL::PP also has the C<broadcastloop> construct, which lets you declare the
code over which PDL should broadcast, and the code that should come before
and after the broadcast loop. Here's a simple example demonstrating the
C<broadcastloop> construct in conjunction with the C<loop> construct:
...
There are two important aspects to remember about broadcastloops. First, you
must not put anything between the C<broadcastloop> and the C<%{> except
...
looks like a broadcastloop in your code, but you've commented it out:
...
The problem is that if you do not indicate where the broadcastloop is supposed to
go, PDL wraps all your code in the broadcastloop logic. However, if PDL sees what
looks like a broadcastloop block, it assumes that you want to be more precise about
where the broadcastloop logic goes. In fact, it even inserts the broadcastloop logic
...
contains the C<loop(n)> block, below the C<#endif>, does not have the broadcastloop
...
something (anything) between the text C<broadcastloop> and the percent block
C<%{>. As already discussed, this will always prevent PDL from identifying
the broadcastloop, which is what you need it to temporarily do in this case.

It may seem that broadcastloops are bad things to be avoided, but broadcastloops
...
that sort of operation, you allocate it before entering the broadcastloop and
...
L<https://en.wikipedia.org/wiki/Numerical_differentiation> . Normally, finite
...
The code within this loop does B<not> actually compute results for all indices
...
In the above section I have explained how to use C<loop> and C<broadcastloop>
to control how PDL::PP presents data to your code, and to control which
sections of code PDL::PP broadcasts over. I have also shown you how to access
...
=item 2. Broadcasting Engine Tricks
...
Using Perl level operations, find a way to manipulate the incoming ndarray
so that you can call C<my_sumover> to get the sum over the entire matrix.
Bonus points if the same technique works for higher dimensional ndarrays.
...
cumulative sum over the entire ndarray, regardless of the ndarray's dimension.
...
This is B<ENORMOUSLY> helpful when you have a syntax error. Without it, the
...
Note: This issue has been addressed with the release of PDL v2.4.11 (May 2012).
...
Here's an example. I am trying to print out the values in an ndarray, but I
...
=for listing exercise-broadcasting
...
C<$sum> and C<$diff>). When you supply a null ndarray (as in the middle
example) or you call the function with the input ndarrays only (as in the
first example), PDL will allocate memory for you. As demonstrated with the
last example, you can supply a pre-allocated ndarray, in which case PDL will
...
Note that you will have to make sure the dimensions of the input ndarrays match this new specification, e.g.:
